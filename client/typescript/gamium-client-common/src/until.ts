...
export class Until {
  static objectLocated(
    locator: Locator,
    options: Partial<FindObjectOptions> = DefaultFindObjectOptions()
  ): ObjectInfoCondition {
    return new ObjectInfoCondition(
      `locate element locator:${JSON.stringify(locator)}`,
      (driver) => {
  ...
  static objectsLocated(
    locator: Locator,
    options: Partial<FindObjectOptions> = DefaultFindObjectOptions()
  ): ObjectInfosCondition {
    return new ObjectInfosCondition(
      `locate element locator:${JSON.stringify(locator)}`,
      (driver) => {
