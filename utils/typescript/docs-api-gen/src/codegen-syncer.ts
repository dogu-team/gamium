...
export class CodeGenSyncer {
  constructor(
    private readonly nodes: CodeGenElemTreeNode[],
    private readonly md: CodeGenMarkdown,
    private readonly typeMdxMapper: CodeGenTypeMdxMapper
  ) {}
  ...
  async sync(): Promise<void> {
    ...
    const contents = [...fileHeader, ...newLines]
      .join("\n")
      .replaceAll("\n\n\n", "\n\n");
    await fs.promises.writeFile(this.md.filePath, contents);
  }

  private findMatchedElemInTree(
    tree: CodeGenElemTreeNode[],
    elem: CodeGenElem
  ): CodeGenElemTreeNode | undefined {
    for (const e of tree) {
      if (
        e.elem.tag === elem.tag &&
        escapeCodeAndLink(e.elem.text) === escapeCodeAndLink(elem.text)
      ) {
        return e;
      }
  ...
  private generateMd(
    befTree: CodeGenElemTreeNode[],
    curTree: CodeGenElemTreeNode[],
    lines: MarkdownLines
  ): string[] {
    ...
    for (const curNode of curTree) {
      ...
      if (befNode) {
        ...
        if (elemLineIdx === -1) {
          throw new Error(
            `elem not found in lines: ${befNode.elem.tag.prefix} ${befNode.elem.text}`
          );
        }
        ...
        follwedLines.push(
          ...lines
            .slice(elemLineIdx + 1, nextElemLineIdx)
            .map((l) => (typeof l === "string" ? l : l.text))
        );
      ...
      const childLines = this.generateMd(
        befNodeChildren,
        curNode.children,
        lines
      );
