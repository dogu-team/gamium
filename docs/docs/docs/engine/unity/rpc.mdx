...
The following C# code can be invoked through the function [executeRpc](../../../api/client/typescript/classes/gamium-client#executeRpc-) in `Gamium Client`.
...
- Don't forget to add the [Preserve Attribute](https://docs.unity3d.com/ScriptReference/Scripting.PreserveAttribute.html) if the function is not being called from anywhere.
