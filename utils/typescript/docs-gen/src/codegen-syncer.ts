...
export class CodeGenSyncer {
  constructor(private readonly ts: CodeGenTypescript, private readonly md: CodeGenMarkdown, private readonly typeMdxMapper: CodeGenTypeMdxMapper) {}

  async sync(): Promise<void> {
    const fileHeader = this.getFileHeaders(this.md.lines);
    const newLines = this.generateMd(this.md.nodes, this.ts.nodes, this.md.lines);
  ...
  private findMatchedElemInTree(tree: CodeGenElemTreeNode[], elem: CodeGenElem): CodeGenElemTreeNode | undefined {
    for (const e of tree) {
      if (e.elem.tag === elem.tag && escapeCodeAndLink(e.elem.text) === escapeCodeAndLink(elem.text)) {
        return e;
      }
  ...
  private generateMd(befTree: CodeGenElemTreeNode[], curTree: CodeGenElemTreeNode[], lines: MarkdownLines): string[] {
    ...
    for (const curNode of curTree) {
      ...
      newLines.push(elemToString(curNode.elem, { mapper: this.typeMdxMapper, mdFilePath: this.md.filePath }));
      newLines.push('');
