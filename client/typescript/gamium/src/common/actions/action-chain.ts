...
import { ActionsClickOptions, DefaultActionClickOptions } from '../options/actions-click-options';
import { ActionsDragOptions, DefaultActionsDragOptions } from '../options/actions-drag-options';
import { ActionsMoveOptions, DefaultActionMoveOptions } from '../options/actions-move-options';
import { DefaultActionScrollOptions } from '../options/actions-scroll-options';
import { DefaultMovePlayerOptions, MovePlayerOptions } from '../options/move-player-options';
import { DefaultSendKeyOptions, SendKeyOptions } from '../options/send-key-options';
import { DefaultSetTextOptions, SetTextOptions } from '../options/set-text-options';
import {
  ActionParam,
  ActionParamSingleT,
  AppQuitParamT,
...
const _dummyParam = new ActionParamSingleT();
type ActionParamTypes = NonNullable<typeof _dummyParam.action>;
...
export const createInputKey = (param: Plain<InputKeyParamT>): ActionPacketTypes => {
  return {
...
export const createInputMouse = (param: Plain<InputMouseParamT>): ActionPacketTypes => {
  return {
    paramEnum: ActionParam.Actions_InputMouseParam,
    param: new InputMouseParamT(param.press, param.button, param.position, param.delta),
  };
};

export const createInputSetText = (param: Plain<InputSetTextParamT>): ActionPacketTypes => {
  return {
...
export const createMovePlayer = (param: Plain<MovePlayerParamT>): ActionPacketTypes => {
  return {
    paramEnum: ActionParam.Actions_MovePlayerParam,
    param: new MovePlayerParamT(param.playerObjectId, param.cameraObjectId, param.position, param.by, param.epsilon, param.checkHeight),
  };
};

export const createAppQuit = (param: Plain<AppQuitParamT>): ActionPacketTypes => {
  return {
...
export class ActionChain {
  constructor(private gamiumService: GamiumService, private actions: ActionPacketTypes[] = []) {}
  ...
  click(position: Vector2, options: Partial<ActionsClickOptions> = DefaultActionClickOptions()): ActionChain {
  ...
  move(position: Vector2, options: Partial<ActionsMoveOptions> = DefaultActionMoveOptions()): ActionChain {
  ...
  drag(fromPosition: Vector2, toPosition: Vector2, options: Partial<ActionsDragOptions> = DefaultActionsDragOptions()): ActionChain {
    ...
    const deltaVector = gl.vec2.div(gl.vec2.create(), gl.vec2.sub(gl.vec2.create(), [toVec2.x, toVec2.y], [fromVec2.x, fromVec2.y]), [deltaCount, deltaCount]);
    this.addAction(createSleep({ ms: optionMixed.intervalMs }));
    for (let i = 1; i < deltaCount - 1; i++) {
      const nextPosition = new Vector2T(fromVec2.x + deltaVector[0] * i, fromVec2.y + deltaVector[1] * i);
      this.addAction(
        createInputMouse({
          press: InputMousePressType.MOVE,
          button: InputMouseButtonCode.LEFT,
  ...
  scroll(position: Vector2, delta: Vector2, options = DefaultActionScrollOptions()): ActionChain {
  ...
  sendKeys(byList: KeyBy[], options: Partial<SendKeyOptions> = DefaultSendKeyOptions()): ActionChain {
    ...
    this.addAction(createInputKey({ press: InputKeyPressType.DOWN, codes: codes }));
    this.addAction(createSleep({ ms: optionMixed.duratiomMs }));
    this.addAction(createInputKey({ press: InputKeyPressType.UP, codes: codes }));
    this.addAction(createSleep({ ms: 33 }));
    return this;
  }

  setText(locator: Locator, text: string, options: Partial<SetTextOptions> = DefaultSetTextOptions()): ActionChain {
  ...
  movePlayer(playerLocator: Locator, cameraLocator: Locator, dest: Vector3, options: Partial<MovePlayerOptions> = DefaultMovePlayerOptions()): ActionChain {
  ...
  async perform(): Promise<ActionResult[]> {
    ...
    for (const action of this.actions) {
      const actionParam = new ActionParamSingleT(action.paramEnum, action.param);
      const builder = new flatbuffers.Builder();
      const offset = actionParam.pack(builder);
      builder.finish(offset);

      const bytes = builder.asUint8Array();
      const str = this.uint8ArrayToString(bytes);
      actionsArray.push(str);
    }
    const res = await this.gamiumService.request(createActions({ actions: actionsArray }));
  ...
  private checkActionReturn(res: Awaited<ReturnType<ActionChain['perform']>>): void {
