...
export function elemToString(
  elem: CodeGenElem,
  converter?: { mapper: CodeGenTypeMdxMapper; mdFilePath: string }
): string {
  let text = elem.text;
  if (converter) {
...
export function linkiFyCodeBlock(
  param: string,
  mapper: CodeGenTypeMdxMapper,
  mdFilePath: string
): string {
  let ret = "";
    ...
    if ("`" === elem) {
      if (bracketOpenIdx === null) {
        ...
        if (!link) {
          ret += "`" + codeBlock + "`";
          console.log(
            chalk.red(`[codegen-elem] link not found for ${codeBlock}`)
          );
        } else {
